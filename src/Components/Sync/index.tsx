import React from 'react';
import { CircularProgress, Box, Grid } from '@material-ui/core';
import "./sync.scss";
import { Requests } from 'Services';
import pouchdb from 'pouchdb';

const Sync = () => {

    const me:any = React.useRef();
    const product:any = React.useRef();
    const supplier:any = React.useRef();
    const users:any = React.useRef();
    const masterCodes:any = React.useRef();
    const reports:any = React.useRef();
    const log:any = React.useRef();
    
    const [finish,setFinish] = React.useState(false);
    const [message,setMessage] = React.useState('Loading Database...');
    
    const callerList:any={
        me          :   {name: "me",           funct : me},
        product     :   {name: "product",      funct : product},
        supplier    :   {name: "supplier",     funct : supplier},
        users       :   {name: "users",        funct : users},
        masterCodes :   {name: "masterCodes",  funct : masterCodes},
        reports     :   {name: "reports",      funct : reports},
        log         :   {name: "log",          funct : log},
    };

    React.useEffect(()=>{
      run();  
    },[]);

    const run = async() => {
        for(var i=0;Object.keys(callerList).length > i ; i++){
            console.log(i);

            let val = Object.keys(callerList)[i];
            console.log(val);

            let a:any = await callerList[val].funct.current.show({per_page:1000,page:1});
            console.log(a);
            // if(a.status === 200 ){
            //     if(a.data.hasOwnProperty('meta')){
                    
            //     }else{
            //         let db = new pouchdb(val);
            //         await db.put({
            //             _id : a.data.id.toString(),
            //             ...a.data
            //         })  
            //         db.close();
            //     }
            // }
        }
        
    };

    const repeater = () => {

    }


    return (
        <Grid
            container
            spacing={0}
            alignItems="center"
            justify="center"
            style={{ minHeight: "calc(100vh - 120px)" }}
        >
            <Requests.Auth request={me} />
            <Requests.Products request={product} />
            <Requests.Suppliers request={supplier} />
            <Requests.User request={users} />
            <Requests.MasterCodes request={masterCodes} />
            <Requests.Status request={reports} />
            <Requests.Logs request={log} />

            <Grid className="text-center" item xs={6}>
                <CircularProgress size={60} /> <br />
                <div>&nbsp;{message}</div>
            </Grid>
        </Grid>
    )
};

export default Sync;